...
public class UsdtPaymentStrategy implements PaymentStrategy {
    ...
    public ApiResponse processPayment(ApiRequest apiRequest, HttpServletRequest request) {
        ...
        try {
            ...
            log.info("USDT-API提现接口, 商户号: {}, 请求IP: {}", apiRequest.getMerchantCode(), requestIp);
            ...
            //校验请求
            ApiResponse apiResponse = validateRequest(apiRequest, requestIp, merchantInfo, merchantPublicKeyStr, "USDT-API提现接口");
            ...
            try {
                ...
                if (withdrawalApplyReq == null) {
                    log.error("USDT-API提现接口处理失败, 参数校验失败: 请求ip: {}, req: {}, 商户信息: {}", requestIp, apiRequest, merchantInfo);
                ...
                if (!violations.isEmpty()) {
                    ...
                    for (ConstraintViolation<WithdrawalApplyReq> violation : violations) {
                        log.error("USDT-API提现接口处理失败, 参数校验失败: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                ...
                if (!apiRequest.getChannel().equals(withdrawalApplyReq.getChannel())) {
                    log.error("USDT-API提现接口处理失败, 支付类型不一致: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                    return ApiResponse.of(ApiResponseEnum.PARAM_VALID_FAIL, null);
                }

                //使用商户公钥验证签名
                if (!RsaUtil.verifySignature(withdrawalApplyReq, withdrawalApplyReq.getSign(), merchantPublicKey)) {
                    log.error("USDT-API提现接口处理失败, 签名校验失败: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                ...
                if (amount.compareTo(BigDecimal.ONE) < 0) {
                    log.error("USDT-API提现接口处理失败, 金额小于1: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}, 订单金额: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq, amount);
                ...
                if (merchantInfo.getWithdrawalStatus().equals("0")) {
                    //当前商户代付状态未开启
                    log.error("USDT-API提现接口处理失败, 当前商户代付状态未开启: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                ...
                if (merchantRatesConfig == null) {
                    log.error("USDT-API提现接口处理失败, 不存在对应的支付类型配置: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                ...
                if (!isBetween) {
                    //订单金额不在最小金额和最大金额之间
                    log.error("USDT-API提现接口处理失败, 金额超过限制: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                    return ApiResponse.of(ApiResponseEnum.AMOUNT_EXCEEDS_LIMIT, null);
                }
                ...
                if (toAddress.length() != 34 || !tronService.checkAddress(toAddress)) {
                    log.error("USDT-API提现接口处理失败, USDT地址无效: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                ...
                try {
                    ...
                    if (req2) {
                        ...
                        //校验商户USDT余额是否足够
                        if (merchantInfo.getUsdtBalance().compareTo(amountCost) < 0) {
                            log.error("USDT-API提现接口处理失败, 商户余额不足: 订单金额(包括费率+单笔手续费): {}, 商户余额: {}, 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", amountCost, merchantInfo.getBalance(), requestIp, apiRequest, merchantInfo, withdrawalApplyReq);
                        ...
                        lambdaUpdateWrapperMerchantInfo.eq(MerchantInfo::getCode, merchantInfo.getCode())  // 指定更新条件 商户号
                                .set(MerchantInfo::getUsdtBalance, merchantInfo.getUsdtBalance().subtract(allAmount)) // 指定更新字段 (减少商户余额 - 总金额)
                                .set(MerchantInfo::getPendingUsdtBalance, merchantInfo.getPendingUsdtBalance().add(allAmount)); // 指定更新字段 (增加交易中金额 + 总金额)
                        ...
                        if (!save) {
                            log.error("USDT-API提现接口处理失败: 生成商户代付订单失败，触发事务回滚。 订单信息: {}, req: {}", merchantPaymentOrders, withdrawalApplyReq);
                            // 抛出运行时异常
                            throw new RuntimeException("USDT-API提现接口处理失败: 生成商户代付订单失败，触发事务回滚。");
                        }
                        ...
                        if (save) {
                            ...
                            log.info("USDT-API提现接口订单提交成功, 请求ip: {}, 请求明文: {}, 返回明文: {}", requestIp, withdrawalApplyReq, withdrawalApplyVo);
                            ...
                            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                                ...
                                public void afterCommit() {
                                    ...
                                    log.info("USDT-API提现接口订单提交成功, 订单号: {}, 订单状态: {} 执行事务同步回调 请求ip: {}, 请求明文: {}, 返回明文: {}", merchantPaymentOrders.getPlatformOrder(), merchantPaymentOrders.getOrderStatus(), requestIp, withdrawalApplyReq, withdrawalApplyVo);
                                    ...
                                    if (PaymentOrderStatusEnum.HANDLING.getCode().equals(merchantPaymentOrders.getOrderStatus())){
                                        ...
                                        TaskInfo taskInfo = new TaskInfo(platformOrder, TaskTypeEnum.USDT_PAYMENT_ORDER.getCode(), System.currentTimeMillis());
                                        //发送处理USDT代付订单的MQ
                                        rabbitMQService.sendUsdtPaymentOrderMessage(taskInfo);
                                    }else{
                                    ...
                                    redisTemplate.opsForValue().set("paymentOrderSign:" + merchantPaymentOrders.getPlatformOrder(), paymentOrderSign, 3, TimeUnit.DAYS);
                                }
                            });
                            ...
                            //提交失败
                            log.error("USDT-API提现接口订单提交失败, 请求ip: {}, 请求明文: {}, 请求密文: {}", requestIp, withdrawalApplyReq, apiRequest);
                            //手动回滚
                    ...
                    //手动回滚
                    TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
                    log.error("USDT-API提现接口订单提交失败 req: {}, e: {}", apiRequest, e);
                } finally {
                    //释放锁
                ...
                //手动回滚
                TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
                log.error("USDT-API提现接口订单提交失败, 数据重复 e: {}", e.getMessage());
                return ApiResponse.of(ApiResponseEnum.DATA_DUPLICATE_SUBMISSION, null);
            } catch (BadPaddingException e) {
                //手动回滚
                TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
                log.error("USDT-API提现接口订单提交失败, 解密失败，无效的密文或密钥错误 e: {}", e.getMessage());
