...
public class TrxCollectionStrategy implements CollectionStrategy {
    ...
    public ApiResponse processCollection(ApiRequest apiRequest, HttpServletRequest request) {
        ...
        log.info("TRX-API充值接口, 商户号: {}, 请求IP: {}", apiRequest.getMerchantCode(), requestIp);
        ...
        //校验请求
        ApiResponse apiResponse = validateRequest(apiRequest, requestIp, merchantInfo, merchantPublicKeyStr, "TRX-API充值接口");
        if (apiResponse != null) {
            return apiResponse;
        }

        //分布式锁key ar-wallet-trxProcessCollection
        String key = "uu-wallet-trxProcessCollection";
        ...
        try {
            ...
            if (req) {
                ...
                if (depositApplyReq == null) {
                    log.error("TRX-API充值接口处理失败, 参数校验失败: 请求ip: {}, req: {}, 商户信息: {}", requestIp, apiRequest, merchantInfo);
                ...
                if (!violations.isEmpty()) {
                    ...
                    for (ConstraintViolation<DepositApplyReq> violation : violations) {
                        log.error("TRX-API充值接口处理失败, 参数校验失败: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, depositApplyReq);
                ...
                if (!apiRequest.getChannel().equals(depositApplyReq.getChannel())) {
                    log.error("TRX-API充值接口处理失败, 支付类型不一致: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, depositApplyReq);
                    return ApiResponse.of(ApiResponseEnum.PARAM_VALID_FAIL, null);
                }

                //使用商户公钥验证签名
                if (!RsaUtil.verifySignature(depositApplyReq, depositApplyReq.getSign(), merchantPublicKey)) {
                    log.error("TRX-API充值接口处理失败, 签名校验失败: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, depositApplyReq);
                ...
                if (merchantRatesConfig == null) {
                    log.error("TRX-API充值接口处理失败, 不存在对应的支付类型配置: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, depositApplyReq);
                ...
                if (amount.compareTo(BigDecimal.ONE) < 0) {
                    log.error("TRX-API充值接口处理失败, 金额小于1: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}, 订单金额: {}", requestIp, apiRequest, merchantInfo, depositApplyReq, amount);
                ...
                if (!isBetween) {
                    //订单金额不在最小金额和最大金额之间
                    log.error("TRX-API充值接口处理失败, 金额超过限制: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, depositApplyReq);
                    return ApiResponse.of(ApiResponseEnum.AMOUNT_EXCEEDS_LIMIT, null);
                }
                ...
                if (merchantInfo.getRechargeStatus().equals("0")) {
                    //当前商户代收状态未开启
                    log.error("TRX-API充值接口处理失败, 当前商户代收状态未开启: 请求ip: {}, req: {}, 商户信息: {}, 请求明文: {}", requestIp, apiRequest, merchantInfo, depositApplyReq);
                ...
                if (createMerchantCollectOrder) {
                    ...
                    //支付地址
                    depositApplyVo.setPayUrl(arProperty.getTrxPayUrl() + "?token=" + usdtPaymentToken);
                    ...
                    log.info("TRX-API充值接口订单提交成功, 请求ip: {}, 请求明文: {}, 返回明文: {}", requestIp, depositApplyReq, depositApplyVo);
                    ...
                    //提交失败
                    log.error("TRX-API充值接口订单提交失败, 请求ip: {}, 请求明文: {}, 请求密文: {}", requestIp, depositApplyReq, apiRequest);
                    return ApiResponse.of(ApiResponseEnum.SYSTEM_EXECUTION_ERROR, null);
                }
            } else {
                //没获取到锁 直接返回操作频繁
                log.error("TRX-API充值接口订单提交失败, 获取分布式锁失败, 请求ip: {}, 请求密文: {}", requestIp, apiRequest);
                return ApiResponse.of(ApiResponseEnum.TOO_FREQUENT, null);
            }
        } catch (DataIntegrityViolationException e) {
            //手动回滚
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            log.error("TRX-API充值接口订单提交失败, 数据重复 e: {}", e.getMessage());
            return ApiResponse.of(ApiResponseEnum.DATA_DUPLICATE_SUBMISSION, null);
        } catch (BadPaddingException e) {
            //手动回滚
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            log.error("TRX-API充值接口订单提交失败, 解密失败，无效的密文或密钥错误 e: {}", e.getMessage());
            return ApiResponse.of(ApiResponseEnum.DECRYPTION_ERROR, null);
        } catch (Exception e) {
            //手动回滚
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            log.error("TRX-API充值接口订单提交失败 req: {}, e: {}", apiRequest, e);
        } finally {
            //释放锁
    ...
    private boolean createMerchantCollectOrders(String platformOrder, DepositApplyReq depositApplyReq, BigDecimal amount, MerchantInfo merchantInfo, String requestIp, BigDecimal rates, BigDecimal fixedFee, TronAddress tronAddress) {
        ...
        //汇率 从实时汇率接口获取 根据币种汇率来计算 USDT订单和TRX订单 默认为0
        merchantCollectOrders.setExchangeRates(BigDecimal.ZERO);

        //itoken数量 USDT订单和TRX订单 默认为0
    ...
    public String createPaymentToken(UsdtPaymentInfoBO usdtPaymentInfoBO, long duration) {
        String token = generateTokenPayment("trxPayment" + usdtPaymentInfoBO.getMerchantCode() + usdtPaymentInfoBO.getMemberId(), duration, arProperty.getSecretKey());
