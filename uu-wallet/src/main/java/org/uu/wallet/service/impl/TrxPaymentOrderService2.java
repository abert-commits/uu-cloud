...
public class TrxPaymentOrderService2 {
        ...
        if (StringUtils.isNotBlank(txId)) {
            ...
            while (true) {
                try {
                    ...
                    log.info("TRX代付出款 updatePaymentOrder2, 订单号: {}, txId", merchantPaymentOrders.getPlatformOrder(), txId);
                        ...
                        if (transactionInfo.getResult() == Response.TransactionInfo.code.SUCESS) {
                            ...
                            if (update1 && update2) {
                                ...
                                Boolean updatemerchantInfo = amountChangeUtil.insertOrUpdateAccountChange(
                                        ...
                                        "",
                                        BalanceTypeEnum.TRX.getName()//余额通道
                                );

                                if (!updatemerchantInfo) {
                                    log.error("TRX代付出款失败: 更新商户信息失败，触发事务回滚。 订单信息: {}", merchantPaymentOrders);
                                    // 抛出运行时异常
                                    throw new RuntimeException("TRX代付出款失败: 更新商户信息失败，触发事务回滚。");
                                }
                                ...
                                log.info("TRX代付出款 循环获取交易记录成功 订单已成功, 订单号: {}", merchantPaymentOrders.getPlatformOrder());
                            }
                        } else {
                            //更新代付钱包交易状态 失败
                            ...
                            if (update1 && update2) {
                                ...
                                lambdaUpdateWrapperMerchantInfo.eq(MerchantInfo::getCode, merchantInfo.getCode())  // 指定更新条件 商户号
                                        .set(MerchantInfo::getTrxBalance, merchantInfo.getTrxBalance().add(allAmount)) // 指定更新字段 (增加商户余额 + 总金额)
                                        .set(MerchantInfo::getPendingTrxBalance, merchantInfo.getPendingTrxBalance().subtract(allAmount)); // 指定更新字段 (减少交易中金额 - 总金额)
                            ...
                            log.info("TRX代付出款 循环获取交易记录成功 订单已失败, 订单号: {}", merchantPaymentOrders.getPlatformOrder());
                        }

                        log.info("TRX代付出款 退出循环获取交易记录");
                        break;
                    } else {
                        log.error("TRX代付出款 循环获取交易记录失败 txId: {}", txId);
                    }
                } catch (Exception ex) {
                    log.error("循环获取 交易结果 失败, ex: {}", ex);
                    //手动回滚
            ...
            log.info("TRX代付出款 txId为null, 请人工进行确认交易状态 updatePaymentOrder2, 订单号: {}, txId", merchantPaymentOrders.getPlatformOrder(), txId);
        }
        return true;
    }
}
