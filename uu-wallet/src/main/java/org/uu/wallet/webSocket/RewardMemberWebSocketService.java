...
@Slf4j
@Component
@ServerEndpoint(value = "/websocket/getRewardMembers/{userId}", configurator = CustomConfigurator.class)
//@DependsOn("redisConfig")
public class RewardMemberWebSocketService {
...
//    }
    ...
    private void init() {
        ...
        if (this.redisTemplate != null) {
            this.redisPublisher = new RedisPublisher(this.redisTemplate, "rewardMembers");
            ...
            if (this.redisContainer != null) {
                this.redisContainer.addMessageListener(new MessageListener() {
                    ...
                    public void onMessage(Message message, byte[] pattern) {
                        ...
                        //log.info("[推送获奖会员]收到频道上的消息时, message: {}", message);

                        // 当收到频道上的消息时执行
                        handleMessageFromRedis(new String(message.getBody()));
                    }
                }, new ChannelTopic("rewardMembers"));
            } else {
                log.error("[推送获奖会员失败] redisTemplate 为null");
            }
        } else {
            log.error("[推送获奖会员失败] redisContainer 为null");
        }
    }
    ...
    private void handleMessageFromRedis(String message) {
        ...
        //log.info("[推送获奖会员]处理从Redis接收的消息, message: {}", message);
        ...
        if (JsonUtil.isValidJSONObjectOrArray(jsonMessage)) {
            //log.info("[推送获奖会员]处理从Redis接收的消息, 消息为json格式, message: {}", jsonMessage);
            //将消息进行群发
            GroupSending(jsonMessage);
        } else {
            log.error("[推送获奖会员]处理从Redis接收的消息 解析JSON失败, message: {}, jsonMessage: {}", message, jsonMessage);
        }
    ...
    public boolean AppointSending(String message) {
        try {
            // 将消息发布到Redis频道
            redisPublisher.publish("rewardMembers", message);
     ...
     */
    private static ConcurrentHashMap<String, RewardMemberWebSocketService> webSocketSet = new ConcurrentHashMap<>();
    ...
    public void OnOpen(Session session, @PathParam(value = "userId") String userId) {
        ...
        log.info("[推送获奖会员]webSocket连接成功，当前连接人数为：{}, userId: {}", webSocketSet.size(), userId);
...
//        GroupSending(userId + " 来了");
    ...
    public void OnClose() {
        ...
        log.info("[推送获奖会员][WebSocket] 退出成功，当前连接人数为：={}", webSocketSet.size());
...
//        log.info("[推送获奖会员] WebSocket 收到消息：{}", message_str);
...
//                //该会员已经断开连接了
//                log.error("[推送获奖会员]webSocket发送指定消息失败: 会员id不存在(已断开连接)");
//                return false;
//            }
//            webSocketSet.get(userId).session.getBasicRemote().sendText(message);
//            log.info("[推送获奖会员]webSocket发送指定消息成功: userId: {}, message: {}", userId, message);
//            return true;
//        } catch (Exception e) {
//            log.info("[推送获奖会员]webSocket发送指定消息失败: userId: {}, message: {}", userId, message);
//            e.printStackTrace();
//            return false;
//        }
//    }
