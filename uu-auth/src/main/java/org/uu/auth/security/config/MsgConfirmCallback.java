...
import org.uu.auth.security.entity.CustomCorrelationData;
            ...
            if (customData != null) {
                if (ack) {
                    switch (customData.getQueueName()) {
                        case RabbitMqConstants.WALLET_MEMBER_LOGIN_LOG_QUEUE:
                            //钱包项目-登录成功
                            logger.info("记录前台会员登录日志 消息发送成功: " + JSON.toJSONString(customData, SerializerFeature.WriteMapNullValue));
                            break;
                        default:
                    ...
                    logger.info("记录前台会员登录日志: 消息发送失败: " + JSON.toJSONString(customData, SerializerFeature.WriteMapNullValue));
                }
            } else {
                logger.error("MQ消息确认 记录前台会员登录日志 CustomCorrelationData 为null...");
            }
        } else {
            logger.error("MQ消息确认 记录前台会员登录日志 correlationData 格式不正确...");
        }
    }
}